...
public class PopularPopupServiceImpl implements PopularPopupService {
    ...
    public PopularPageResponse getPopularPopup(int limit, String cursor) {
        ...
        if (cursor == null) {
            Map<String, Object> head = popularSnapshotRepository.findLatestSnapshotHead()
                    .orElse(null);
        ...
        List<PopularPopupRow> popularPopupRows = (cursor == null)
                ? popularSnapshotRepository.fetchFirstPage(snapShotId, limitPlusOne)
                : popularSnapshotRepository.fetchNextPage(snapShotId, cursorView, cursorPostId, limitPlusOne);
        ...
        if(hasNextPage && !popularPopupRows.isEmpty()){
            ...
            PopularCursor next = new PopularCursor(
                    snapShotId,
                    last.viewCountSnapshot(),
                    last.id(),
                    expEpochSeconds
            );
        ...
        return new PopularPageResponse(
                popularPopupRows,
                new PageInfo(hasNextPage, nextCursor, snapShotId)
        );
    }
}
