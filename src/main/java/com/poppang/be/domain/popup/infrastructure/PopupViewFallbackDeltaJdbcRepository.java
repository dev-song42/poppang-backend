 ...
 *
 * <p>- bulk insert (micro-batch) - batch claim + merge + processed 마킹 (중복 처리 방지)
 */
@Repository
...
public class PopupViewFallbackDeltaJdbcRepository {
  ...
  /**
   * 처리 대상 claim: 과거 minute bucket만 잡아 batch_id를 고정한다. 여러 인스턴스에서 동시에 실행되어도 같은 row를 중복 claim하기 어렵다.
   */
  ...
  /**
   * claim된 batch_id를 popup_total_view_count에 합산. (INSERT ... SELECT ... ON DUPLICATE KEY UPDATE)로
   * 1쿼리 merge.
   */
